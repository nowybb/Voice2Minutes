...
| | Uvicorn | ASGI 서버 |
| AI | Return Zero STT API | 음성 파일 전사(STT) 및 화자 분리 |
| | Google Gemini API | AI 회의 요약, 결정사항, Action Items 및 키워드 추출 |
| | Pydantic | 데이터 검증 및 Gemini Structured Output(JSON Schema) 처리 |
...
- 매우 짧은 음성 파일은 AI가 Action Items나 결정 사항을 충분히 추출하지 못할 수 있습니다.
